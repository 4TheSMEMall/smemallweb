...
import {
  ...
  ChartBarIcon, BuildingIcon, ArrowTrendingUpIcon,
  ClockIcon, ExclamationTriangleIcon, LightBulbIcon, CheckCircleIcon, BoltIcon,
...
const bhcFeatures = [
  { Icon: ChartBarIcon,        title: "Business Health Score", desc: "Get a 0–100 score across 6 key business dimensions." },
  { Icon: BuildingIcon,        title: "Lender Matching",       desc: "See which banks and lenders your business qualifies for." },
  { Icon: ClipboardIcon,       title: "Detailed Report",        desc: "Download a full PDF report you can share with lenders." },
  { Icon: ArrowTrendingUpIcon, title: "Improvement Roadmap",    desc: "Get specific actions to improve your score over time." },
];
                ...
                {bhcFeatures.map((f) => (
                  <div key={f.title} className="flex items-start gap-3 p-3 rounded-xl bg-gray-50">
                    <div className="w-9 h-9 bg-white rounded-lg border border-gray-100 flex items-center justify-center flex-shrink-0">
                      <f.Icon className="w-4.5 h-4.5 text-navy-900" />
                    </div>
                    <div>
        ...
        {latest && (
          <>
            ...
            <div className="bg-white rounded-3xl shadow-card border border-gray-100 overflow-hidden">
              <div className="relative bg-navy-900 p-6 sm:p-8 overflow-hidden">
                ...
                <div className="relative flex flex-col sm:flex-row sm:items-center gap-6 sm:gap-8">
                  <ScoreRing percentage={latest.percentage} size={140} strokeWidth={9} className="flex-shrink-0 mx-auto sm:mx-0">
                    <div className="text-center">
                      <p className="text-4xl font-extrabold text-white leading-none tabular-nums">{latest.percentage}</p>
                      <p className="text-gray-500 text-[11px] font-bold uppercase tracking-widest mt-1">/ 100</p>
                    </div>
                  </ScoreRing>

                  <div className="flex-1 text-center sm:text-left">
                    <p className="text-gray-400 text-xs font-medium uppercase tracking-widest mb-2">Latest Score</p>
                    <div className="flex items-center justify-center sm:justify-start gap-2 flex-wrap mb-3">
                      <span className={`px-3.5 py-1.5 rounded-full border text-sm font-bold ${statusColor(latest.status)}`}>
                        {latest.status}
                      </span>
                      {scoreChange !== null && (
                    ...
                    <p className="text-gray-500 text-sm leading-relaxed max-w-sm">
                      {latest.percentage >= 70
                        ? "Your score qualifies you for most lender partners on the platform."
                        : `${70 - latest.percentage} points away from unlocking commercial lender matching.`}
                    </p>
                  </div>
                </div>
              </div>
                      ...
                      <div key={s.name} className={`rounded-xl p-4 border transition-colors ${isWeak ? "bg-amber-50 border-amber-200" : "bg-gray-50 border-transparent"}`}>
                        <div className="flex items-center justify-between mb-2">
                          <div className="flex items-center gap-1.5">
                            {isWeak && <ExclamationTriangleIcon className="w-3.5 h-3.5 text-amber-500" />}
...
function LoanReadinessBanner({ percentage, status }: { percentage: number; status: string }) {
  if (percentage >= 70) {
    return (
      <div className="bg-emerald-50 border border-emerald-200 rounded-2xl p-5 flex items-start gap-4">
        <div className="w-10 h-10 bg-emerald-500 rounded-xl flex items-center justify-center flex-shrink-0"><CheckCircleIcon className="w-5 h-5 text-white" /></div>
        <div>
  ...
  if (percentage >= 45) {
    return (
      <div className="bg-amber-50 border border-amber-200 rounded-2xl p-5 flex items-start gap-4">
        <div className="w-10 h-10 bg-amber-400 rounded-xl flex items-center justify-center flex-shrink-0"><BoltIcon className="w-5 h-5 text-white" /></div>
        <div>
  ...
  return (
    <div className="bg-red-50 border border-red-200 rounded-2xl p-5 flex items-start gap-4">
      <div className="w-10 h-10 bg-red-500 rounded-xl flex items-center justify-center flex-shrink-0"><ExclamationTriangleIcon className="w-5 h-5 text-white" /></div>
      <div>
